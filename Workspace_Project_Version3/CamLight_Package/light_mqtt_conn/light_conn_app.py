...
#     sys.exit(1)
MQTT_LIGHT_TOPIC = os.getenv('MQTT_LIGHT_TOPIC', None)
if MQTT_LIGHT_TOPIC is None:
    logging.error('MQTT_LIGHT_TOPIC undefined.')
    sys.exit(1)
...
def on_message(client, userdata, msg):
    ...
    light_status.insert_one({
        'status': msg_data["status"],
        'location': msg_data["location"]
    })
    light_evts.insert_one({
        'brightness': msg_data["brightness"],
        'location': msg_data["location"],
