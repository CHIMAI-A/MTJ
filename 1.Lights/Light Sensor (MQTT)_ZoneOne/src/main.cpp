...
if(topicStr == "zoneOne/emergency_light/control"){
  if (message == "ON"){
    Serial.print("Led ON");
...
if(topicStr == "esp32_S3_Box_ZoneOne/cam/detect"){
  ...
  if (person>0){
    ...
    client.publish("esp32_dev_ZoneOne/light/brightness",String(brightness).c_str());

  } else{
    if (lightValue>500){
       analogWrite(LED_PIN, 50);
       client.publish("esp32_dev_ZoneOne/light/brightness",String(50).c_str());
    }else{
      analogWrite(LED_PIN, 0);
      client.publish("esp32_dev_ZoneOne/light/brightness",String(0).c_str());
    }
...
void reconnect() {   // for MQTT connection
  ...
  while (!client.connected()) {
    ...
    if (client.connect("esp32_dev_ZoneOne_Client")) {
      ...
      client.subscribe("zoneOne/emergency_light/control");
    } else {
