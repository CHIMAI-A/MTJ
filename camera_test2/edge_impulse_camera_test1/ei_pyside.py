...
class EspCamWidget(QtWidgets.QWidget):
    ...
    def grab_image(self):
        ...
        if result is not None:
            ...
            print(f"result after jpeg_image_read ============> {result}")
            self.rpc_master.get_bytes(self.buf, jpg_sz)
            print(str(datetime.now() - Tstart) + ': ' + str(result))
            #img = cv2.imread("test.jpg")
            test = np.frombuffer(self.buf,dtype=np.uint8)
            print(f"test ===========> {test}")
            test1 = cv2.imdecode(test,cv2.IMREAD_COLOR)
            print(f"test ===========> {test1}")
            self.img = test1 #cv2.imdecode(np.frombuffer(self.buf, dtype=np.uint8), cv2.IMREAD_COLOR)
    ...
    def upload_data(self):
        ...
        res = requests.post('https://ingestion.edgeimpulse.com/api/testing/files',
                            headers=headers,
                            files=payload)
